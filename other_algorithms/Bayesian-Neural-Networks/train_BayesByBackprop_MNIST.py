...
for i in range(epoch, nb_epochs):
    ...
    #i, nb_epochs, kl_cost_train[i], pred_cost_train[i]*1000, err_train[i]), end="")
    #cprint('r', '   time: %f seconds\n' % (toc - tic))

    print("it %d/%d, Jtr_ELBO = %f, err = %f, " % (
    i, nb_epochs, kl_cost_train[i] + pred_cost_train[i], err_train[i]), end="")
    cprint('r', '   time: %f seconds\n' % (toc - tic))
    elbo_lst.append(kl_cost_train[i] + pred_cost_train[i])
...
np.save("BNN_elbo.npy", np.array(elbo_lst))
...
save_object(net.get_weight_samples, models_dir+'/state_dicts.pkl')
